...
Sec. 8-13bb and 8-13cc}) and Regional Housing Needs allocation
requirements
(\href{https://cga.ct.gov/2026/sup/chap_124b.htm\#sec_8-13dd}{C.G.S.
Sec. 8-13dd}). The guidance outlines datasets, analytical methods, and
mapping standards that can be used to produce a consistent, statewide
...
\begin{center}
\pandocbounded{\includegraphics[keepaspectratio]{images/components.png}}
\end{center}
...
\item
  ...
  and
\item
  \textbf{Regional Needs Assessment allocations} (C.G.S. Sec. 8-13dd),
  prepared exclusively by COGs
\end{itemize}
...
inventory for the Housing Growth Plans and Regional Needs Assessment
allocations (see \href{outputs.qmd}{Section 2} for details on outputs).
As described above, these two outputs share a common statutory
...
For both the Housing Growth Plans and the Regional Needs Assessment, the
developable land inventory process is primarily analytical in nature,
involving the identification and documentation of areas that meet the
statutory definition of developable land through the application of
knowledge, data, and methodology. Regarding the Housing Growth Plan maps
specifically, the map itself is not the mechanism by which developable
areas are identified, nor does it constitute a determination of a
property's development potential. Rather, it serves as a visual
representation of the underlying analysis, allowing the results of that
analysis to be verified and understood spatially. Because the
developable land inventory is a spatial exercise, a map demonstrates
that the statutory criteria under C.G.S. Sec. 8-13aa were applied
...
inventory within that framework, users should refer to the
\href{https://experience.arcgis.com/experience/057c27cda2d848edbcb1ffac4a1b8898}{OPM
Office of Responsible Growth 2025 Act Concerning Housing Growth}
website.
...
developable land under C.G.S. Sec. 8-13aa; (2) satisfies the
\href{outputs.qmd}{required outputs} described in this document,
including the required maps; and (3) is adequately documented so that
the methodology and results are transparent and reproducible.

\bookmarksetup{startatroot}

\chapter{Required Outputs}\label{required-outputs-1}

Per these guidelines, the following represent the outputs expected for
the Housing Growth Plan report and Regional Housing Needs allocation.
The approach used to generate these outputs is at the discretion of the
COG or municipality; sections \href{stat_def.qmd}{3} and
\href{vac_redev.qmd}{4} suggest a framework along with supporting data
...
\href{https://experience.arcgis.com/experience/057c27cda2d848edbcb1ffac4a1b8898/page/Homepage}{2025
Act Concerning Housing Growth web page} from OPM.

Per the Housing Growth Plan Guidelines, the following outputs are
required for inclusion in the Housing Growth Plan report as part of the
...
program}\label{regional-housing-needs-program}

\ul{C.G.S. Sec. 8-13dd}: The results of this process of identifying
qualifying developable land per C.G.S. Section 8-13aa are used in
calculating the Regional Housing Needs allocation under C.G.S. Section
8-13dd(c)(3).
...
assumption commercially reasonable. Municipalities and COGs should apply
local market knowledge and professional judgment when assessing
feasibility, consistent with the statutory standard.
...
evaluation during preparation of the Housing Growth Plan and Regional
Needs Assessment using locally informed, commercially reasonable
assumptions.
...
\KeywordTok{OR}\NormalTok{ STATE\_USE\_DESCRIPTION }\KeywordTok{LIKE}\NormalTok{ \%REC\%}\StringTok{\textquotesingle{}}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{Connecticut Greenways}\label{connecticut-greenways}
...
and supports analyses of the development potential of identified areas.
The inventory produced during this phase serves as a foundation for
further study, not a final determination of development potential.

This section describes suggested queries for identifying parcels within
...
not be financially viable to remediate to residential standards.
Municipalities and COGs should carefully review brownfield sites using
local knowledge and supplemental data before including them as
potentially redevelopable, and should be mindful of environmental
...
\item
  ...
  \pandocbounded{\includegraphics[keepaspectratio]{images/kmeans.png}}
...
Maps are only required as part of the Housing Growth Plans. They are not
a required element of the Regional Needs Assessment. This section
focuses on the maps in the Housing Growth Plans.
...
definition of a developable land inventory and to provide a high-level
view of the entire municipality. Other elements of the Housing Growth
Plan (such as the Parcel \& Zone Identification section) apply a more
detailed suitability analysis that incorporates additional planning
factors and considerations to determine whether specific areas are well
suited or poorly suited for development. The developable land inventory
is a starting point that feeds into that later, more detailed process.
...
\hyperref[required-outputs-1]{Section 2}) and statutory intent.
